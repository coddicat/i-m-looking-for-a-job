...
import { CollectionReference, Timestamp, collection, doc, getFirestore, onSnapshot, setDoc, updateDoc } from "firebase/firestore";
...
const collRef = collection(firestoreDb, "users") as CollectionReference<UserEntity>;
...
export const useAuthStore = defineStore('currentUser', () => {
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            console.log(`User is signed in as ${isAnonymous ? 'anonymous' : 'not anonymous'}, uid: ${uid}`);
